...
/**
 * Módulo para formatação de feedback da análise de redações
 */

import { ScoreCategories } from './types';

/**
 * Formata o feedback para redações com nota zero
 * @param motivo Motivo pelo qual a redação foi zerada
 * @returns Feedback formatado
 */
export const formatZeroFeedback = (motivo: string): string => {
  ...
  } else if (motivo.includes("impropério") || motivo.includes("desenho") || motivo.includes("ofensivo")) {
...
/**
 * Formata o feedback para análise de redações
 * @param text Texto da redação
 * @param categories Pontuação por competência
 * @param totalScore Pontuação total
 * @param zeroReason Motivo de nota zero (opcional)
...
export const formatFeedback = (
  ...
  // Se a redação foi zerada, usar formatação específica
  if (zeroReason) {
  ...
  // Adicionar comentários específicos para a competência 1
  let comp1Comentarios = [`### Recomendações para melhorar:`];
  
  if (categories.competencia1 < 160) {
    comp1Comentarios.push(`- Revise as regras de concordância verbal e nominal.`);
    comp1Comentarios.push(`- Atente-se à pontuação, principalmente ao uso de vírgulas.`);
  }
  ...
  if (categories.competencia1 < 80) {
    comp1Comentarios.push(`- Estude as regras básicas de sintaxe da língua portuguesa.`);
    comp1Comentarios.push(`- Consulte um dicionário em caso de dúvidas na grafia das palavras.`);
  }
  ...
  // Adicionar comentários específicos para a competência 2
  let comp2Comentarios = [`### Recomendações para melhorar:`];
  
  if (categories.competencia2 < 160) {
    comp2Comentarios.push(`- Enriqueça seu texto com repertório sociocultural (filosofia, literatura, história, ciências, etc.).`);
    comp2Comentarios.push(`- Certifique-se de abordar o tema de forma direta, sem tangenciamentos.`);
  }
  if (categories.competencia2 < 120) {
    comp2Comentarios.push(`- Estruture melhor seu texto em introdução, desenvolvimento e conclusão.`);
    comp2Comentarios.push(`- Evite copiar trechos dos textos motivadores.`);
  }
  if (categories.competencia2 < 80) {
    comp2Comentarios.push(`- Estude a estrutura do texto dissertativo-argumentativo.`);
    comp2Comentarios.push(`- Leia sobre o tema antes de escrever, para ampliar seu repertório.`);
  }
  ...
  // Adicionar comentários específicos para a competência 3
  let comp3Comentarios = [`### Recomendações para melhorar:`];
  
  if (categories.competencia3 < 160) {
    comp3Comentarios.push(`- Organize melhor seus argumentos, desenvolvendo-os de forma mais aprofundada.`);
    comp3Comentarios.push(`- Apresente exemplos concretos para fundamentar seus argumentos.`);
  }
  if (categories.competencia3 < 120) {
    comp3Comentarios.push(`- Articule melhor as informações, evitando contradições entre seus argumentos.`);
    comp3Comentarios.push(`- Vá além dos textos motivadores, trazendo informações de seu conhecimento.`);
  }
  if (categories.competencia3 < 80) {
    comp3Comentarios.push(`- Defina claramente seu ponto de vista logo na introdução.`);
    comp3Comentarios.push(`- Selecione informações relevantes e pertinentes ao tema.`);
  }
  ...
  // Adicionar comentários específicos para a competência 4
  let comp4Comentarios = [`### Recomendações para melhorar:`];
  
  if (categories.competencia4 < 160) {
    comp4Comentarios.push(`- Utilize mais conectivos para articular ideias (portanto, entretanto, além disso, etc.).`);
    comp4Comentarios.push(`- Evite repetições desnecessárias, usando pronomes e sinônimos.`);
  }
  if (categories.competencia4 < 120) {
    comp4Comentarios.push(`- Dê mais atenção às transições entre parágrafos.`);
    comp4Comentarios.push(`- Diversifique o vocabulário e os recursos coesivos.`);
  }
  if (categories.competencia4 < 80) {
    comp4Comentarios.push(`- Estude os mecanismos de coesão textual.`);
    comp4Comentarios.push(`- Revise a estrutura dos períodos, preferindo frases mais claras e diretas.`);
  }
  ...
  // Adicionar comentários específicos para a competência 5
  let comp5Comentarios = [`### Recomendações para melhorar:`];
  
  if (categories.competencia5 < 160) {
    comp5Comentarios.push(`- Detalhe mais sua proposta de intervenção, especificando agentes, ações, meios e efeitos.`);
    comp5Comentarios.push(`- Articule melhor a proposta com os argumentos desenvolvidos no texto.`);
  }
  if (categories.competencia5 < 120) {
    comp5Comentarios.push(`- Desenvolva uma proposta viável e específica, não genérica.`);
    comp5Comentarios.push(`- Explique como sua proposta poderia ser implementada na prática.`);
  }
  if (categories.competencia5 < 80) {
    comp5Comentarios.push(`- Sempre inclua uma proposta de intervenção na conclusão.`);
    comp5Comentarios.push(`- Mencione explicitamente os agentes responsáveis pela intervenção.`);
  }
