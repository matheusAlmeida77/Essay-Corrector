...
const ReviewPage = () => {
  ...
  const [theme, setTheme] = useState(state?.theme || state?.analysisResult?.theme || '');
  const [title, setTitle] = useState(state?.title || state?.analysisResult?.title || '');
  ...
  const formatCompetenciaLabel = (key: string) => {
    switch(key) {
      case 'competencia1': return 'Competência 1: Domínio da norma padrão';
      case 'competencia2': return 'Competência 2: Compreensão da proposta';
      case 'competencia3': return 'Competência 3: Organização de argumentos';
      case 'competencia4': return 'Competência 4: Mecanismos linguísticos';
      case 'competencia5': return 'Competência 5: Proposta de intervenção';
      default: return key;
    }
  };
  
  const isZeroScore = analysisResult.zeroReason || (analysisResult.score && analysisResult.score.total === 0);
  
  const mockGrammarErrorCounts = {
    spelling: analysisResult.corrections.filter(c => c.type === 'spelling').length || 2,
    grammar: analysisResult.corrections.filter(c => c.type === 'grammar').length || 1,
    punctuation: analysisResult.corrections.filter(c => c.type === 'punctuation').length || 3,
    agreement: analysisResult.corrections.filter(c => c.type === 'agreement').length || 0,
    syntax: analysisResult.corrections.filter(c => c.type === 'syntax').length || 1,
    formality: analysisResult.corrections.filter(c => c.type === 'formality').length || 0,
  };
  ...
  const mockArgumentAnalysis = {
    hasThesis: true,
    hasConclusion: true,
    argumentCount: Math.max(1, Math.round(analysisResult.statistics.paragraphsCount * 0.7) - 1),
    counterArgumentPresent: analysisResult.score.categories.competencia3 >= 180,
    connectivesUsed: ["portanto", "além disso", "por outro lado", "assim", "entretanto"].slice(0, Math.min(5, analysisResult.statistics.connectivesCount)),
    topicCoherence: Math.min(95, analysisResult.score.categories.competencia3 / 2),
            ...
            <div>
              <Card className="border border-gray-200 h-full">
                <CardContent className="p-6">
                  {isZeroScore && (
                    <div className="mb-6">
                      <div className="p-4 bg-red-50 border-l-4 border-red-500 rounded-lg mb-4">
                        <h3 className="text-xl font-bold text-red-600 mb-2">REDAÇÃO COM NOTA ZERO</h3>
                        <p className="font-medium text-red-700">
                          Motivo: {analysisResult.zeroReason}
                        </p>
                        <p className="mt-2 text-sm">
                          De acordo com os critérios do ENEM, esta redação recebeu nota zero pelo motivo indicado acima.
                          Para mais informações, consulte as orientações sobre os critérios de avaliação do ENEM.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {!isZeroScore && (
                    <div className="mb-6">
                      <h2 className="text-xl font-bold mb-4">Pontuação por Competência ENEM</h2>
                      <div className="grid grid-cols-1 gap-4 mb-4">
                        {analysisResult.score && analysisResult.score.categories && Object.entries(analysisResult.score.categories).map(([category, score]) => {
                          let numericScore = 0;
                          ...
                          switch(category) {
                            case 'competencia1':
                              ...
                              categoryName = 'Competência 1 - Domínio da Norma Culta';
                              break;
                            case 'competencia2':
                              progressColor = 'bg-green-500';
                              borderColor = 'border-green-500';
                              categoryName = 'Competência 2 - Compreensão da Proposta';
                              break;
                            case 'competencia3':
                              progressColor = 'bg-amber-500';
                              borderColor = 'border-amber-500';
                              categoryName = 'Competência 3 - Organização de Argumentos';
                              break;
                            case 'competencia4':
                              progressColor = 'bg-purple-500';
                              borderColor = 'border-purple-500';
                              categoryName = 'Competência 4 - Uso de Mecanismos Linguísticos';
                              break;
                            case 'competencia5':
                              progressColor = 'bg-rose-500';
                              borderColor = 'border-rose-500';
                              categoryName = 'Competência 5 - Proposta de Intervenção';
                              break;
                            default:
                          ...
                          return (
                            <div key={category} className={`p-3 bg-gray-50 rounded-lg border-l-4 ${borderColor}`}>
                      ...
                      <div className="p-4 bg-sesi-red/10 rounded-lg mb-4">
                        <div className="flex items-center justify-between">
                          ...
                          <p className="text-xl font-bold">{analysisResult.score && analysisResult.score.total ? (analysisResult.score.total).toFixed(0) : "0"}</p>
                        </div>
                      </div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg text-sm">
                          <div dangerouslySetInnerHTML={{ __html: analysisResult.feedback }} />
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-red-100 text-red-800">
                          <span className="w-3 h-0.5 bg-red-500 mr-1 decoration-wavy"></span> Erros de ortografia
                        </span>
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-yellow-100 text-yellow-800">
                          <span className="w-3 h-0.5 bg-yellow-500 mr-1 decoration-wavy"></span> Erros gramaticais
                        </span>
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-green-100 text-green-800">
                          <span className="w-3 h-0.5 bg-green-800 mr-1"></span> Conectivos
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        <div className="p-2 bg-gray-50 rounded border">
                          ...
                          <p className="text-sm font-medium">{analysisResult.statistics.paragraphsCount}</p>
                        </div>
                        <div className="p-2 bg-gray-50 rounded border">
                          <p className="text-xs text-gray-500">Palavras</p>
                          <p className="text-sm font-medium">{analysisResult.statistics.wordsCount}</p>
                        </div>
                        <div className="p-2 bg-gray-50 rounded border">
                          <p className="text-xs text-gray-500">Linhas (real)</p>
                          <p className="text-sm font-medium">{analysisResult.statistics.linesCount || countLines(analysisResult.text)}</p>
                        </div>
                        <div className="p-2 bg-gray-50 rounded border">
                          <p className="text-xs text-gray-500">Linhas (estimativa)</p>
                          <p className="text-sm font-medium">{analysisResult.statistics.estimatedLines || Math.ceil(analysisResult.statistics.wordsCount / 10)}</p>
                        </div>
                        <div className="p-2 bg-gray-50 rounded border">
                          <p className="text-xs text-gray-500">Conectivos</p>
                          <p className="text-sm font-medium">{analysisResult.statistics.connectivesCount}</p>
                        </div>
                        <div className="p-2 bg-gray-50 rounded border">
                          <p className="text-xs text-gray-500">Repertório</p>
                          <p className="text-sm font-medium">{analysisResult.socioculturalReferences?.type || "Não identificado"}</p>
                        </div>
                      </div>
                      
                      <div>
                        <h3 className="font-medium mb-2">Desempenho por Competência</h3>
                          ...
                          {chartType === 'radar' ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                              ...
                              >
                                ...
                                <Tooltip formatter={(value) => [`${value} pontos`, 'Pontuação']} />
                                <Legend />
                                <Bar dataKey="pontos" name="Pontuação" fill="#8884d8" />
                              </BarChart>
                            </ResponsiveContainer>
                          )}
                        </div>
                ...
                <TabsContent value="analysis" className="space-y-6">
                  <Card className="border border-gray-200">
                    <CardContent className="p-6 space-y-6">
                      <div>
                        <h2 className="text-xl font-bold mb-4">Análise Detalhada</h2>
                        
                        <GrammarErrorDetails errorCounts={mockGrammarErrorCounts} />
                        
                        <div className="mt-6">
                          <ArgumentAnalysisDetails argumentAnalysis={mockArgumentAnalysis} />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-gray-500">
                            Deixe em branco para usar a nota sugerida pela IA ({analysisResult.score && analysisResult.score.total ? analysisResult.score.total.toFixed(1) : "0.0"})
                          </p>
                        </div>
                      </div>
